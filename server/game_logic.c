...
static bool check_coin_collision(server_t *restrict server, int player_index,
                                 int coin_index) {
  ...
  bool y_matches =
      (int)p_y == (int)coin_y || ((int)p_y == 10 && (int)coin_y == 9);
...
static void check_player_collision(server_t *restrict server,
                                   int player_index) {
  if (server->game.coins_pos == NULL) return;
